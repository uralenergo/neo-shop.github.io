...
<?php
    if (!empty($cat) && !empty($type)){
        $querycat = "AND type='$cat' AND type_tovara='$type'";
    }
    else{
        if (!empty($type)){
...
<ul id="sorting-list">
<li><a href="view_cat.php?cat='.$cat.'&type='.$type.'&sort=price-asc">От дешёвых к дорогим</a></li>
<li><a href="view_cat.php?cat='.$cat.'&type='.$type.'&sort=price-desc">От дорогих к дешёвым</a></li>
...
<li><a href="view_cat.php?cat='.$cat.'&type='.$type.'&sort=title">По алфавиту</a></li>
</ul>
</li>
</ul>
</div>
  ...
  $row = mysql_fetch_array($result);
   ...
   do{
    ...
    echo '
        ...
        echo '<h3>В данной категории нет товаров</3>';
    }
